import { doc, getDocs, query, where, collection, orderBy, limit, runTransaction } from 'firebase/firestore';
...
export const unlockNextWordForUser = async (currentWordId, wordType, userId) => {
    try {
        // Run a transaction to ensure atomicity
        const result = await runTransaction(db, async (transaction) => {
            // Step 1: Fetch the current word
            const currentWordRef = doc(db, 'users', userId, 'words', currentWordId);
            const currentWordDoc = await transaction.get(currentWordRef);
            ...
            if (!snapshot.empty) {
                // Step 4: Unlock the next word
                const nextWordDoc = snapshot.docs[0];
                const nextWordRef = doc(db, 'users', userId, 'words', nextWordDoc.id);
